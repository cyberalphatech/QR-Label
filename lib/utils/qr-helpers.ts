...
// Button action types from the original PHP helper
...
export function getButtonActions(published: boolean): ButtonAction[] {
  const baseActions: ButtonAction[] = ["edit", "qr", "nutrition", "recycling", "certificate", "details", "duplicate"]

  if (!published) {
    baseActions.push("publish")
  }
